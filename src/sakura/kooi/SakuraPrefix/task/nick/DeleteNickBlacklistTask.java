...
public class DeleteNickBlacklistTask implements Runnable {
	...
	public void run() {
		try {
			if (SakuraPrefix.getPrefixManager().delNickBlacklist(value)) {
				callback.sendMessage("§d§lSakuraPrefix §7>> §aNick黑名单已删除: §r"+value);
			} else {
				callback.sendMessage("§d§lSakuraPrefix §7>> §cNick黑名单不存在: §r"+value);
			}
